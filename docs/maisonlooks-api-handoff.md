...
3. Pages: `index.html` (hero + category bar from `topbar-cats.js`: **fill one row** with as many links as fit, then **More** for overflow; edit `LINKS` in that file; image-only grid; scroll loads more, `limit=200`), `products.html` (full catalog UI + API-filled category dropdown, `limit=48` + Load more + `?category=`), `outfits.html` (outfits grid).
