...
3. Pages: `index.html` (hero + category bar: shoe/clothing links visible, rest under **更多分类** `<details>`; image-only grid; scroll loads more, `limit=200`; `?category=` from top bar), `products.html` (full catalog UI, `limit=48` + Load more + `?category=`), `outfits.html` (outfits grid).
