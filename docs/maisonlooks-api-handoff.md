...
3. Pages: `index.html` (hero + category bar + image-only product grid + Load more; `?category=` from top bar), `products.html` (full catalog UI + `?category=`), `outfits.html` (outfits grid).
