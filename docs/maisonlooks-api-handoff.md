...
3. Pages: `index.html` (hero + category bar with wrap layout + apparel-related categories first + image-only grid; scroll loads more, `limit=200`; `?category=` from top bar), `products.html` (full catalog UI, `limit=48` + Load more + `?category=`), `outfits.html` (outfits grid).
